...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	// handle file upload
	$file = $_FILES["file"];
	$pdf_file_path = APP_PATH."/storage/".Uuid::uuid4()->__toString().".pdf";
	$uploadOk = 1;
	...
	// if everything is ok, try to upload file
	  ...
	  if (move_uploaded_file($file["tmp_name"], $pdf_file_path)) {
	    echo "The file <strong>". htmlspecialchars( basename($file["name"]) ). "</strong> has been uploaded.";
	  } else {
	...
	$pdfContent = File::getContents($pdf_file_path);
	$pdf = bin2hex( $pdfContent['content'] );
